require "fileutils"
require "pry"
@new_dylib_path = File.join("s", "d", "d")
...
def fix_dylib_for_file(file)
	...
	if results.is_a?(String) && results != "" && !results.include?("is not an object file")  && !results.include?("Assertion failed:")
		...
		id_path = File.join("s", "d", expanded.split(File.join(@current_dir, "s","d"))[-1])
...
folders = [File.expand_path(File.join(File.dirname(File.expand_path($0)), "..", "s", "d"))]
